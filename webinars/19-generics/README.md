...
- Для базовых типов-значений работают как обычные функции без дженериков
...
* Для типов-поинтеров, таких как `*int`, `*float64`, `[]int`, `chan int` используется тип GCShape.

* По в скомпилированном коде GCShape выглядит как обычная ссылка фиксированного размера, 
...
* Переписывать API, основанные на интерфейсах, переориентируя их на работу с дженериками.
